...
class TradeExecutor:
    ...
    def execute_trade(self, trade: dict, direction: str):
        ...
        if direction == 'trade':
            ...
            req = requests.post((api_url + uri_path), headers = headers, data = payload, timeout = 1) # post trade
            result = req.json() # response                
            self.log_trade_execution(result, 'submitted') if 'code' not in result.keys() else self.log_trade_execution(result['msg'], 'failed')
            return True
    ...
    def log_trade_execution(self, result, status):
        ...
        if status == 'submitted':
            ...
            order_event = OrderEvent(symbol, order_id, ExecutionType[status], OrderStatus[status])
...
############## self.manager.give_trade(result) # give trade back to manager
    ...
    async def _reconnect_ws(self):
        ...
        self._async_client = await AsyncClient.create(self.api_key, self.api_secret, testnet = self.testnet)
    ...
    async def _listen_execution_forever(self):
        ...
        while ws.open:
            ...
            if update_type == 'ORDER_TRADE_UPDATE':
                ...
                order_event = OrderEvent(symbol, order_id, ExecutionType[execution_type], OrderStatus[order_status])
                order_event.side = Side[side]
